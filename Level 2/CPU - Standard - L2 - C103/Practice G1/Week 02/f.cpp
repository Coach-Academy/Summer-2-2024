...
int main() {
    khaled
    cin>>n;
    vector<int>zeros , negative , postive;
    for(int i = 0;i < n;i++) {
        int x;cin>>x;
        if(!x)zeros.emplace_back(x);
        else if(x > 0)postive.emplace_back(x);
        else negative.emplace_back(x);
    }
    if(postive.size() == 0) {
        postive.emplace_back(negative.back());
        negative.pop_back();
        postive.emplace_back(negative.back());
        negative.pop_back();
    }
    if(negative.size() % 2 == 0){
        zeros.emplace_back(negative.back());
        negative.pop_back();
    }
    cout<<negative.size()<<" ";
    for(auto &val:negative)cout<<val<<" ";
    cout<<line<<postive.size()<<' ';
    for(auto &val:postive)cout<<val<<" ";
    cout<<line<<zeros.size()<<" ";
    for(auto &val:zeros)cout<<val<<" ";
