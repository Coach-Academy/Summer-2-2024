...
signed main() {
    ...
    while(n--) {
        ...
        if(op == "insert") {
            ...
            int x;cin>>x;
            while(!pq.empty() && pq.top() < x)pq.pop() , ans.emplace_back("removeMin");
            if(pq.empty() || pq.top() > x)ans.emplace_back("insert " + to_string(x)) , pq.push(x);
            ans.emplace_back(op + " " + to_string(x));
        }else {
            // remove min
            if(pq.empty()) {
                pq.push(10);
                ans.emplace_back("insert " + to_string(10));
            }
            pq.pop();
            ans.emplace_back(op);
        }
